...
set _REALPATH=%~dp0
set _WRAPPED_APP=eftlink-rest-api
set _WRAPPED_APP_NAME=eftlink-rest-api
set _WRAPPER_CONF="%_REALPATH%%_WRAPPER_BASE%\conf\%_WRAPPED_APP_NAME%.conf"
set PLATFORM=windows
::echo Real path is %_REALPATH%
set JAVA_HOME=c:\jre
set JAVA=%JAVA_HOME%\bin\%_WRAPPED_APP%.exe
...
if not exist "%PIDDIR%\%_WRAPPED_APP_NAME%.pid" goto notask

for /f "tokens=*" %%a in (%PIDDIR%\%_WRAPPED_APP_NAME%.pid) do set XPID=%%a
tasklist /FI "PID eq %XPID%" |find "%_WRAPPED_APP%.exe"
REM echo %ERRORLEVEL%
if %ERRORLEVEL% == 0 (
       cls
       ECHO Another Instance of %_WRAPPED_APP_NAME% is already running from %_REALPATH%.
       exit /b 3
       goto :eof
       )
if %ERRORLEVEL% == 1 (
       echo Stale pid file detected, pid file removed.
       del %PIDDIR%\%_WRAPPED_APP_NAME%.pid
       )
...
:console
SET my_title=restapi Console (%RANDOM%)
