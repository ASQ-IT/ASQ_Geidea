...
set _REALPATH=%~dp0
set _WRAPPER_CONF="%_REALPATH%%_WRAPPER_BASE%\conf\eftlink.conf"
set _WRAPPED_APP=eftlink
set PLATFORM=windows
::echo Real path is %_REALPATH%
set JAVA_HOME=c:\jre
set JAVA=%JAVA_HOME%\bin\eftlink.exe
set PIDDIR=%_REALPATH%tmp

:: Move any JVM crash logs into the log directory.
IF EXIST "%_REALPATH%hs_pid????.log"  move "%_REALPATH%hs_pid????.log" "%_REALPATH%log"
...
for /f "tokens=*" %%a in (.\tmp\%_WRAPPED_APP%.pid) do set XPID=%%a
...
:console
SET my_title=EFTLink Console (%RANDOM%)
